...
console.error('🚀 Initializing RSS News MCP Server v3.0...');
...
if (transportMode === 'stdio') {
    ...
    console.error('📡 MCP Transport: stdio (Normal MCP)');
    const transport = new StdioServerTransport();

    (async () => {
        await server.connect(transport);
        console.error('✅ RSS News MCP Server v3.0 başarıyla başlatıldı!');
        console.error('📋 Kullanılabilir Araçlar (26):');
        console.error('  🔷 Temel Araçlar:');
        console.error('  • rss_add - RSS feed ekle');
        console.error('  • rss_list - Feedleri listele');
        console.error('  • rss_update - Feedleri güncelle');
        console.error('  • rss_news - Haber getir');
        console.error('  • rss_search - Haber ara');
        console.error('  • rss_delete - Feed sil');
        console.error('  🔷 Gelişmiş Analiz:');
        console.error('  • rss_breaking - Son dakika haberleri');
        console.error('  • rss_duplicates - Mükerrer haberleri bul');
        console.error('  • rss_analytics - Analitik & istatistikler');
        console.error('  • rss_trends - Trend analizi');
        console.error('  • rss_sentiment_analysis - Duygu analizi');
        console.error('  🔷 İçerik & Çeviri:');
        console.error('  • rss_translate - Haber çevir (AI destekli)');
        console.error('  • rss_media - Medya çıkar');
        console.error('  • rss_full_content - Tam içerik çıkar');
        console.error('  🔷 Karşılaştırma & Doğrulama:');
        console.error('  • rss_compare - Feed karşılaştır');
        console.error('  • rss_cross_verify - Çapraz doğrulama');
        console.error('  🔷 Dışa Aktarma:');
        console.error('  • rss_export - Dışa aktar (JSON/CSV/XML)');
        console.error(' • rss_daily_digest - Günlük özet');
        console.error('  • rss_opml - OPML import/export');
        console.error('  🔷 Akıllı Özellikler:');
        console.error('  • rss_recommend - Feed önerileri');
        console.error('  • rss_auto_categorize - Otomatik kategorize');
        console.error('  • rss_credibility_score - Güvenilirlik skoru');
        console.error('  🔷 Yönetim & Takip:');
        console.error('  • rss_notification_setup - Webhook bildirimleri');
        console.error('  • rss_bookmark - Bookmark yönetimi');
        console.error('  • rss_schedule - Otomatik güncelleme');
        console.error('  • rss_health_monitor - Feed sağlık izleme');
        console.error('\n💡 Bu sunucu stdio transport ile çalışıyor.');
        console.error('   Claude Desktop veya diğer MCP istemcileri ile kullanılabilir.');
    })().catch((error) => {
